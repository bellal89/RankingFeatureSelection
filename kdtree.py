...
def square_distance(pointA, pointB):
    ...
    distance = 0
    dimensions = max(pointA.max_feature_id, pointB.max_feature_id)
    for dimension in range(dimensions):
        distance += (pointA.get_feature(dimension) - pointB.get_feature(dimension))**2
    return distance

class KDTreeNode():
    def __init__(self, feature_vector, left, right):
        self.feature_vector = feature_vector
        self.point = feature_vector
        self.left = left
        self.right = right
...
class KDTreeNeighbours():
    ...
    def add(self, node):
        ...
        for i, e in enumerate(self.current_best):
            ...
            if e[1] > sd:
                self.current_best.insert(i, [node, sd])
                self.calculate_largest()
                return
        # append it to the end otherwise
        self.current_best.append([node, sd])
        self.calculate_largest()
...
class KDTree():
    ...
    def __init__(self, data):
        def build_kdtree(feature_vectors, depth):
            # code based on wikipedia article: http://en.wikipedia.org/wiki/Kd-tree
            if not feature_vectors:
                return None

            # select axis based on depth so that axis cycles through all valid values
            axis = depth % self.features_count # assumes all points have the same dimension

            # sort point list and choose median as pivot point,
            # TODO: better selection method, linear-time selection, distribution
            feature_vectors.sort(key=lambda vector: vector.get_feature(axis))
            median = len(feature_vectors)/2 # choose median

            # create node and recursively construct subtrees
            node = KDTreeNode(feature_vector=feature_vectors[median],
                              left=build_kdtree(feature_vectors[0:median], depth+1),
                              right=build_kdtree(feature_vectors[median+1:], depth+1))
            return node
        
        self.features_count = max([v.max_feature_id for v in data])
    ...
    def query(self, feature_vector, t=1):
        statistics = {'nodes_visited': 0, 'far_search': 0, 'leafs_reached': 0}
        
        def nn_search(node, feature_vector, t, depth, best_neighbours):
            if node == None:
                return
            ...
            axis = depth % self.features_count
            ...
            # compare feature_vector and point of current node in selected dimension
            # and figure out which subtree is farther than the other
            if feature_vector.get_feature(axis) < node.point.get_feature(axis):
            ...
            nn_search(near_subtree, feature_vector, t, depth+1, best_neighbours)
            ...
            if (node.point.get_feature(axis) - feature_vector.get_feature(axis))**2 < best_neighbours.largest_distance:
                #statistics['far_search'] += 1
                nn_search(far_subtree, feature_vector, t, depth+1, best_neighbours)
            
            return
        
        # if there's no tree, there's no neighbors
        if self.root_node != None:
            neighbours = KDTreeNeighbours(feature_vector, t)
            nn_search(self.root_node, feature_vector, t, depth=0, best_neighbours=neighbours)
